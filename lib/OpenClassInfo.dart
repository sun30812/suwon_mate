...
class OpenClassInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FavoriteButton(subjectCode: '${arg['subjtCd']}-${arg['diclNo']}',),
    );
  }
}
...
class FavoriteButton extends StatefulWidget {
  ...
  const FavoriteButton({Key? key, required String subjectCode}) : _subjectCode = subjectCode, super(key: key);
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (!snapshot.hasData) {
          return SuwonButton(icon: Icons.star_outline, buttonName: '즐겨찾기 추가', onPressed: null,);
        } else {
          if (_isGetFirst) {
          ...
          if (!_isAddFavorite) {
            return SuwonButton(icon: Icons.star_outline, buttonName: '즐겨찾기 추가', onPressed: () {
              setState(() {
                _isAddFavorite = true;
            ...
            return SuwonButton(icon: Icons.star, buttonName: '즐겨찾기에서 제거', onPressed: () {
              setState(() {
